...
> 💡 **个人备注**：macOS 上首次运行若提示无法验证开发者，需前往「系统设置 → 隐私与安全性」手动允许运行。
>
> 💡 **个人备注2**：如果代理启动后浏览器访问 HTTPS 页面出现证书错误，需在软件内重新安装证书，或手动将证书添加到系统信任列表。
>
> 💡 **个人备注3**：代理端口默认为 8899，如果该端口被占用可在配置文件中修改为其他端口（如 8080），修改后需重启软件生效。
>
> 💡 **个人备注4**：在 macOS 上，启动代理后若发现 Safari 无法正常访问网页，可尝试在「系统设置 → 网络 → 代理」中确认代理地址和端口是否已正确填写为 127.0.0.1:8899。
